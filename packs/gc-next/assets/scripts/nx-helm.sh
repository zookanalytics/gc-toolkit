#!/bin/sh
# nx-helm.sh — the cross-rig human-attention board plus the pick-a-row
# launcher, REWIRED for the continuation-group conversation model
# (tk-h9pq5; spec §6). The gc-next port of the live gc-helm.sh: the
# board, flag, clear, takeaway, and react verbs are carried verbatim;
# `open` and `takeaway --release` change meaning:
#
#   open  → pick a row; if the bead's continuation group has a live
#           member, attach to it (warm); else FILE A TURN routed to the
#           converse pool and let demand spawn the session (cold). No
#           host is resumed or created — bead-host is retired.
#   takeaway --release → ending the conversation: stops routing new
#           turns and lets any warm session drain. No host to put down.
#
# Usage:
#   nx-helm [board] [--json] [--limit=N] [--timeout=SECONDS] [--refresh]
#   nx-helm open  <bead-id>                 land in the bead's conversation (attach warm / file a turn cold)
#   nx-helm flag  <bead-id> --reason "..."  raise this bead onto the board
#   nx-helm clear <bead-id>                 lower it again (the handled row leaves)
#   nx-helm takeaway <bead-id> "<text>" [--by …] [--release]  set the board-visible takeaway headline
#
#   flag  → a bead's LLM (or the operator) raises its own bead onto the
#           board by setting `gc.attention` — a flag is the natural
#           precursor to a turn; pick-a-row files it.
#   board → the operator glances the ranked rows (liveness glyph, row
#           cap, cached gather) — carried unchanged from the live board.
...
# The converse pool template this launcher files turns at (bare; rig-
# qualified per bead below). Matches mol-nx-turn's contract.
NX_CONVERSE_TEMPLATE="${GC_NX_CONVERSE_TEMPLATE:-gc-next.converse}"
...
Usage:
  ...
  nx-helm flag  <bead-id> --reason "..."  raise this bead onto the board
  nx-helm clear <bead-id>                 lower it again (the handled row leaves)
  nx-helm react <bead-id> [--reason "..."]  sling a first reaction (self-heals a takeaway-less row)
  nx-helm takeaway <bead-id> "<text>" [--by host|proactive] [--release]  set the board-visible takeaway headline (--release = end the conversation: stop routing turns, let any warm session drain)
...
# Resolve the sibling proactive tool. nx-outride.sh is the gc-next port
# (PORTS.md); until it lands, fall back to the live pack's tool (valid
# through the overlap window — the repo root holds both packs).
SCRIPT_PATH=$(readlink -f "$0" 2>/dev/null || echo "$0")
SCRIPT_DIR=$(dirname "$SCRIPT_PATH")
PROACTIVE_TOOL="${GC_PROACTIVE_TOOL:-$SCRIPT_DIR/nx-outride.sh}"
[ -x "$PROACTIVE_TOOL" ] || PROACTIVE_TOOL="$SCRIPT_DIR/../../../../tools/gc-proactive.sh"
# (bead-host tool reference removed: retired with the host model, tk-h9pq5 Q4)
...
# ── Verb: open ───────────────────────────────────────────────────────
# The pick-a-row launcher, rewired (tk-h9pq5 §6; spec §6): file-or-attach
# a turn. WARM — the bead's continuation group has a live member: attach
# the operator to that session. COLD — no live member: file a turn bead
# routed to the converse pool (the mol-nx-turn shape, inlined) and let
# demand spawn the session; the operator lands on it when it's up (or
# re-picks the row, now warm). No host is resumed or created.
cmd_open() {
    bead="${1:-}"
    case "$bead" in -h|--help) usage; exit 0 ;; "") echo "$PROG: open needs <bead-id>" >&2; usage; exit 2 ;; esac

    # Point bd at the bead's rig so group reads + the turn land in the
    # right per-rig ledger even cross-rig (BEADS_DIR pins bd).
    path=$(rig_path_for_bead "$bead")
    db=""; [ -n "$path" ] && [ -d "$path/.beads" ] && db="$path/.beads"
    rig=$(rig_name_for_bead "$bead")

    # WARM probe: is any member of the group in_progress? Its assignee is
    # the live session identity the operator can attach to.
    # shellcheck disable=SC2086
    live=$(gc bd list ${db:+--db "$db"} --status=in_progress --json 2>/dev/null \
        | jq -r --arg g "$bead" \
            '[.[]? | select(.metadata["gc.continuation_group"] == $g and (.assignee // "") != "")] | .[0].assignee // empty' \
            2>/dev/null || true)

    if [ -n "$live" ]; then
        bust_cache
        echo "$PROG: conversation for $bead is warm on $live — landing..." >&2
        if [ -n "${TMUX:-}" ] && tmux has-session -t "$live" 2>/dev/null; then
            tmux switch-client -t "$live" || {
                echo "$PROG: could not switch the tmux client." >&2
                echo "       Switch yourself:  tmux switch-client -t $live" >&2
            }
        else
            gc session attach "$live" || {
                echo "$PROG: warm session $live exists; could not attach from this context." >&2
                echo "       Attach it yourself:  gc session attach $live" >&2
            }
        fi
        return 0
    fi

    # COLD: file the turn (mol-nx-turn's contract — stamp-don't-sling,
    # one key per flag, rig-qualified pool; task_kind=conversation is
    # the reaper-skip tell).
    pool="${rig:+$rig/}$NX_CONVERSE_TEMPLATE"
    # shellcheck disable=SC2086
    turn=$(gc bd create ${db:+--db "$db"} -t task \
        --title "turn: $bead — operator pick-a-row" \
        -d "The operator picked this bead off the board. Rebuild the subject's slice, prep, and hold." \
        --json 2>/dev/null | jq -r '.id // .[0].id // empty')
    [ -n "$turn" ] && [ "$turn" != "null" ] || {
        echo "$PROG: open: could not create a turn bead for '$bead' (does it exist in rig '${path:-?}'?)" >&2
        exit 4
    }
    # shellcheck disable=SC2086
    gc bd update "$turn" ${db:+--db "$db"} \
        --set-metadata "gc.routed_to=$pool" \
        --set-metadata "gc.continuation_group=$bead" \
        --set-metadata "task_kind=conversation" >/dev/null 2>&1 \
        || { echo "$PROG: open: created $turn but could not stamp its routing — remove it or stamp by hand" >&2; exit 4; }
    # shellcheck disable=SC2086
    gc bd dep add "$turn" "$bead" --type=parent-child ${db:+--db "$db"} >/dev/null 2>&1 || true
    bust_cache
    echo "$PROG: cold — turn $turn filed for $bead, routed to $pool; demand will spawn the conversation." >&2
    echo "       Re-pick the row in a moment to land warm, or:  gc session attach <the spawned session>" >&2
}
