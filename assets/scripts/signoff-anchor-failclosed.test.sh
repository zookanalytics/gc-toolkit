...
# gate <review_for_gate> <work> <fail?> -> echo the snippet's exit code.
# exit 0 == transition PROCEEDS; non-zero == fail-closed defer.
gate() {
  : > "$FAKE_META"
  if REVIEW_FOR_GATE="$1" WORK="$2" FAIL_ANCHOR_WRITE="$3" bash "$TMP/run.sh" >/dev/null 2>&1; then
    echo 0
  else
    echo "$?"
  fi
