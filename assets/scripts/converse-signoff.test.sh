...
# stamp lands EMPTY on a minority of visits — 7 of the 74 ever filed when this
# was written, across three filers, including BOTH visits in_progress city-wide
# on 2026-08-24. An empty group turns the deliberate (NOGROUP) fallback above
# from a rare benign case into THE failure case, silently disabling this guard
# for exactly the turn it was written to catch: an unrelated visit vacuumed onto
# a live sitting. A visit records its subject twice, and the `tracks` edge has
# held where the stamp did not (su-ab9je).
