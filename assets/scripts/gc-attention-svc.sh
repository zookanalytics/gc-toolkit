#!/usr/bin/env bash
# gc-attention-svc.sh — proxy_process launcher for the Attention Canvas backend.
#
# The supervisor invokes this as a workspace-service `command` (declared in a
# city-scoped [[service]]; see services/attention/README.md). cmd.Dir is the
# city root, so the relative command path resolves there. This wrapper exists
# because the Go binary lives in the rig (rigs/gc-toolkit/services/attention)
...
MOD="$(cd "$HERE/../../services/attention" && pwd)"
...
if [ "$need_build" -eq 1 ]; then
    ( cd "$MOD" && "$GO" build -o "$BIN" ./cmd/attention-svc )
fi
