...
#
# gate-ensure.sh stamps the declared default only when check_set is absent or
# empty, and merge-push stamps a non-empty value on every anchor it
# transitions. The formula's own default is therefore the one fresh work gets,
# and a formula that disagrees with the registry gates every new anchor on the
# wrong set with nothing downstream able to notice.
#
