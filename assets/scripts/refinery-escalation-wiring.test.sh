#!/usr/bin/env bash
# Thin wiring check: every escalation in the refinery patrol goes through
...
grep -q 'escalate\.sh' "$TOML" \
  && ok "refinery patrol escalates through escalate.sh" \
  || bad "refinery patrol never references escalate.sh"
...
echo
echo "refinery-escalation-wiring: $PASS passed, $FAIL failed"
[ "$FAIL" -eq 0 ]
