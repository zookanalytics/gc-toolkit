#!/bin/sh
# tmux-pick-zortex.sh — Gas City Zortex-board picker (pick-a-row → land).
#
# Usage: tmux-pick-zortex.sh [--city-path <path>]
#
...
#
# Reads the ranked board from `gc-zortex.sh --json` (cached, so a
# glance is sub-second) and renders it as a tmux display-menu. Picking a
# row runs `gc-zortex.sh open <bead>`, which resumes-or-materializes
...
ATTN="$(dirname "$SCRIPT")/gc-zortex.sh"
[ -x "$ATTN" ] || { gcmux display-message -d 4000 "Zortex: gc-zortex.sh not found"; exit 0; }
...
if [ "$COUNT" -eq 0 ]; then
    gcmux display-message -d 4000 "Zortex board: nothing needs you. (Nothing floats.)"
    exit 0
fi
...
gcmux display-menu -T " Zortex — what needs you " -x C -y C -- "$@"
