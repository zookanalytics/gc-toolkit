...
# merge, pr-facts, convoy-graduate, review-sweep, pr-stack) — the posture arm runs BEFORE
...
for a in gate-ensure.sh pre-open-rebase.sh pr-open.sh merge.sh pr-facts.sh convoy-graduate.sh review-sweep.sh pr-stack.sh; do mkarm "$a"; done
: > "$ARM_LOG"
out=$(drive); rc=$?
eq "$rc" 0 "a clean pass exits 0"
order=$(cut -d'|' -f1 "$ARM_LOG" | paste -sd, -)
eq "$order" "gate-ensure.sh,pre-open-rebase.sh,pr-open.sh,pr-facts.sh,merge.sh,pr-facts.sh,convoy-graduate.sh,review-sweep.sh,pr-stack.sh" "the arms ran in the load-bearing order"
