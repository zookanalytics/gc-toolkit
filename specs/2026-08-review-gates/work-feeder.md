...
not a bolt-on cadence arm. The review side it twins landed without its
convergence half ([scope.md](scope.md)), so the human door below is
`escalate.sh` and nothing here depends on a review-round counter.
