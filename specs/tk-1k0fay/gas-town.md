...
| Order artifact (digest cooldown/24h/dog-pool spec) | `orders/digest-generate.toml` — order definition for `mol-digest-generate` declaring cooldown (24h) and dog-pool assignment | `rigs/gascity/examples/gastown/packs/gastown/orders/digest-generate.toml@9a01227` | 2026-05-24 |
...
| `mol-refinery-patrol` | Refinery patrol — validate identity, inbox, find assigned work, rebase, run tests, handle failures, merge (direct or PR), patrol summary, self-pour next wisp | `rigs/gascity/examples/gastown/packs/gastown/formulas/mol-refinery-patrol.toml@9a01227` | Poured as root-only wisp on refinery startup per rig; each iteration self-pours next | 9 steps (`validate-identity`, `check-inbox`, `find-work`, `rebase`, `run-tests`, `handle-failures`, `merge-push`, `patrol-summary`, `next-iteration`) | Partial — merge mechanics, rebase, PR validation are generic to git-flow merge processors; but rejection-path assignee `${GC_RIG:+$GC_RIG/}{{binding_prefix}}polecat` and rig/binding-prefix variable names are Gastown-specific |
...
`pack.toml` declares `[[named_session]]` entries for
`mayor`/`deacon`/`boot`/`witness`/`refinery` binding those templates
to scope (`city`/`rig`) and mode (`always`/`on_demand`). `polecat`
is an agent/pool template (not a named session) — it is sized as a
rig-scope pool rather than declared as a `[[named_session]]`. The
