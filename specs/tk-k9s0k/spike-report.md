# Spike report: `mechanik-thread` as pack-defined agent (`tk-k9s0k`)

> **Naming update (2026-05-11):** This document was originally written when the agent was named `mechanik-side`. Renamed to `mechanik-thread` pre-merge per the design conversation under `gascity:gc-h1gxg`. All paths and identifiers below reflect the post-rename name.
...
**v2 incorporates mechanik review (PR #9, `tk-q6oye`):** removed `max_active_sessions` cap (operator-unbounded concurrency), dropped the `[[named_session]] mechanik-thread` block (pool semantics, not singleton), sharpened role fragment with active-action / not-system-of-record framing.
...
- `agents/mechanik-thread/agent.toml` — new agent config
...
`agents/mechanik-thread/agent.toml`:
...
thread's identity even when the template file is the canonical
mechanik's.
...
```bash
gc --city <test-city> prime --strict mechanik-thread
```
...
- Ends with the thread-role fragment (appended via `append_fragments`).
- Thread-role fragment renders `{{ .BindingName }}.mechanik` correctly as
...
- `{{.AgentBase}}` resolves to the per-instance identity (e.g.,
  `gc-toolkit.mechanik-thread-1`), so concurrent instances do not collide.
...
```
/home/zook/loomington/.gc/worktrees/gc-toolkit/mechanik-thread/gc-toolkit.mechanik-thread-1/
/home/zook/loomington/.gc/worktrees/gc-toolkit/mechanik-thread/gc-toolkit.mechanik-thread-2/
...
```
...
QualifiedName = Dir + "/" + BindingName + "." + Name
              = "gc-toolkit/gc-toolkit.mechanik-thread"
```
...
```
"gc-toolkit/gc-toolkit.mechanik-thread-1"
```

So spawned sessions get `GC_AGENT = gc-toolkit/gc-toolkit.mechanik-thread-N`.
This is the form that appears in bead `assignee` / `created_by` columns
when the thread closes a bead or sends mail.
...
- The thread, if it runs `gc hook` from inside its session, gets
  no work assigned.
...
- `gc sling gc-toolkit/gc-toolkit.mechanik-thread <bead>` fails with a
  legible error instead of silently writing `gc.routed_to`.
- Beads cannot be routed-to-pool for mechanik-thread. The canonical
  mechanik remains the only `mechanik`-shaped sling target.

Verified:
```
$ sh -c "echo 'mechanik-thread is operator-spawned only; not a sling target' >&2; exit 1" ; echo $?
mechanik-thread is operator-spawned only; not a sling target
1
```
...
**Why this should work.** Threads are distinct agent identities
in pack config (`mechanik-thread`, not `mechanik`). The `gc session reset`
...
thread's session is owned by `mechanik-thread` and survives a reset
of `mechanik`.

`wake_mode = "resume"` keeps the thread's conversation across
...
gc agent list 2>/dev/null || gc status | grep mechanik-thread
# Expect: mechanik-thread template visible, 0 instances active.

gc session new gc-toolkit/gc-toolkit.mechanik-thread
# Expect: spawns mechanik-thread-1, attach-capable.
...
# Then back outside:
gc bd list --created-by 'gc-toolkit/gc-toolkit.mechanik-thread-1' --limit 1
# Expect: the bead, with created_by = "gc-toolkit/gc-toolkit.mechanik-thread-1".

# Verification 5 — never a sling target
TEST_BEAD=$(gc bd create "spike-test: sling probe" -t task --json | jq -r .id)
gc sling gc-toolkit/gc-toolkit.mechanik-thread "$TEST_BEAD"
# Expect: error "mechanik-thread is operator-spawned only; not a sling target".

# Cleanup
gc bd close "$TEST_BEAD"

# Core acceptance — canonical reset
# With mechanik-thread-1 still alive:
gc session reset gc-toolkit.mechanik
# Expect: canonical mechanik session restarts; mechanik-thread-1 unaffected,
# its conversation/transcript intact.
```

## D. Open follow-ups (not addressed here)

- **Namepool.** Threads are numbered (`mechanik-thread-1`, `-2`).
...
- **Thread-thread coordination.** Two threads sharing the same
