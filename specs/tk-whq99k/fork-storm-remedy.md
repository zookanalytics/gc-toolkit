...
Everything past that increment — the shell-invocation surface, the two upstream remedies — is named below and deferred.

## What the finding reports

The `fork-rate` check is advisory and warns at 100 forks/s. As of its most recent recurrence at 2026-09-15T10:07:35Z, this bead's finding had been observed five times since 2026-09-15T05:54:18Z, at 240, 955, 217, 439, and 515 forks/s — every sample above the 100/s threshold, and fluctuating rather than climbing steadily. The check's own diagnosis: a high fork rate, not CPU work, inflates the load average, because the load average counts runnable and uninterruptible tasks; a fork storm reads as high load while the CPU is far from saturated.

The finding attributes the storm to the per-command data plane and asserts that `gc`, `bd`, and `dolt` dominate the fork count while the agents are a rounding error. That per-source split is not derived here: confirming which process forks how often needs a root `bpftrace` on `sched:sched_process_fork`, deferred to the operator checklist below. What the code does establish is the direction — every bead operation on this rig's shell and gctk surfaces forks a `gc` process — and the shape of the surface that produces it. The in-process exception, helm's board and probe paths, is covered below.
...
Every bead read or write from a shell script or from gctk goes through `gc bd`. A linter enforces the shell half (`tools/lint-learned.d/raw-bd-invocation.sh`: a shell script reaches the store through `gc bd`, never by running `bd` itself). The one exception is helm: its Go board and probe paths read the store directly through `BeadsSource` (below), so forking through `gc bd` is a property of the shell and gctk surfaces, not of the rig as a whole. The surface has two very different halves.

**The shell half is the storm.** `git grep -hE 'gc bd ' -- 'assets/scripts/*.sh' ':!assets/scripts/*.test.sh' | wc -l` returns 377 matching lines across 54 non-test scripts in `assets/scripts/`, and that is before `tools/`, `packs/`, and the doctor checks that loop `gc bd` per rig. The `gc bd ` pattern matches text, not only executable calls. Of the 377 lines, 33 are whole-line comments (`git grep -hE '^[[:space:]]*#.*gc bd ' -- 'assets/scripts/*.sh' ':!assets/scripts/*.test.sh' | wc -l`), and dozens more are operator-facing diagnostic strings such as `warn "… Repair: gc bd update …"`. Excluding comments, quoted strings, and heredoc bodies leaves 295 executable invocations across 50 of those scripts, which is the reducible fork surface. Counted as executable invocations, the heaviest scripts are `gc-helm.sh` (48), `pr-facts.sh` (32), `signoff.sh` (19), `gate-ensure.sh` (18), and `finding.sh` (18); these sit at or below their raw match counts, because the most heavily commented scripts carry the most non-executing mentions — `gc-helm.sh` matches 79 lines, 31 of them comments or strings. Each executable invocation forks a `gc` process, and the hot ones run on every patrol, sweep, and review. This half is the dominant fork generator, and no in-process Go library reduces it: a shell script forks a process to read a bead no matter what gctk links.
...
**bd talks to a long-lived Dolt server, not a fresh Dolt per call.** The store runs a managed per-rig `dolt sql-server` in server mode (the tracked `.beads/metadata.json` carries `"dolt_mode": "server"`). `bd` connects to it over the AD-04 port chain. A cold store auto-spawns a server, but in steady state the Dolt process is persistent, so the reducible per-command forks are the `gc` and `bd` processes, not `dolt`.
...
- `tk-4lxad`: runtime tooling appends `dolt.mode: server` to the working copy of the tracked `.beads/config.yaml`, dirtying it and disabling gc-toolkit's auto-fast-forward clean-tree guard. Its subject is that guard defect; the runtime write corroborates the standing per-rig Dolt server this spec relies on, though the tracked source of that mode is `.beads/metadata.json` (above), not the config file.
