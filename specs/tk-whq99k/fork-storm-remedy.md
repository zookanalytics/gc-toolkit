...
**The shell half is the storm.** 377 `gc bd` invocation lines span 54 non-test scripts in `assets/scripts/` alone, before counting `tools/`, `packs/`, and the doctor checks that loop `gc bd` per rig. That count is `git grep -hE 'gc bd ' -- 'assets/scripts/*.sh' ':!assets/scripts/*.test.sh' | wc -l`; the trailing space in the pattern counts calls, not comment or string mentions, and `git grep -lE` with the same pattern lists the 54 files. The heaviest single scripts are `gc-helm.sh` (79), `pr-facts.sh` (34), `signoff.sh` (27), `gate-ensure.sh` (20), and `finding.sh` (18). Each site is a fork of `gc`, and the hot ones run on every patrol, sweep, and review. This half is the dominant fork generator, and no in-process Go library reduces it: a shell script forks a process to read a bead no matter what gctk links.
