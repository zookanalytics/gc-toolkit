...
A `Tile` carries 51 fields, declared in `internal/board/model.go` and mirrored
...
section cluster_key settled pre_open_stalled
```

`updated_at`, `closed_at` and `pr_owed_since` are `omitzero`, and `cluster_key`,
`settled` and `pre_open_stalled` are `omitempty` — the fields a tile may omit. A
source that cannot read `updated_at` (the supervisor backend) omits it on every
row; `closed_at` is present on a `DONE` row and absent from every live one;
`pr_owed_since` is omitted on every row nothing is owed on; `cluster_key` is
absent on a row that is not one of a template's several; `settled` and
`pre_open_stalled` are absent unless the merge anchor is, respectively, a
stood-down gate or a genuine pre-open stall.
...
request the operator owes), `gate` (a person must answer), `stalled` (open work
nothing is moving), `active` (healthy in-flight), `cleanup` (finished, empty or
disposed), `done` (closed) — read in `board.SectionOrder`. It is orthogonal to
`severity`'s how-badly, and both renderers group by it rather than each
re-deriving a split. A merge anchor bands by what its PR phase asks of a person,
not by being a pull request: `classifySection` sends an owed PR to `review`, a
`settled` (stood-down) gate to `cleanup`, a `pre_open_stalled` gate to
`stalled`, and every other PR row — a progressing cadence, a green branch
waiting only on the merge pass — to `active`.
