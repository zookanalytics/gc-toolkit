...
would have argued. Nothing re-derives the branch diff behind triage, so the
row is a rule triage follows, not a backstop that catches it — which is why
the list is short and holds only paths where a wrong change is expensive.
