...
`SWEEP_BUDGET` runs on the sweep clock and honors a fraction on any host;
`CALL_TIMEOUT` and `KILL_AFTER` reach `timeout(1)`, so where a host's
`timeout(1)` parses only whole seconds their fraction is rounded up to the next
whole second — a fraction reaching such a `timeout(1)` errors the first bounded
call and would end the pass. A garbage value
