...
| `check-seed-audit-current` | A prompt fragment moved without `docs/seed-audit/` being regenerated, or the pre-commit hook that regenerates it is not wired in this checkout | `assets/scripts/render-seed-audit.sh && git add docs/seed-audit`; wire the hook once with `assets/scripts/render-seed-audit.sh --install-hook` |
