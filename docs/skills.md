...
gc-toolkit/
├── .claude-plugin/
│   └── marketplace.json        # portability contract (not created yet — see Open item)
...
- **`compatibility` declares the portability axis.** *Recommended* on every
  gc-toolkit skill so the boundary is legible without reading the body — a
  deliberate local convention, not a spec requirement. The Agent Skills spec
  treats the field as optional and says most skills do not need it, and the
  seven skills shipping in `skills/` today (`demo-capture`,
  `filing-documentation`, `gc-demo-script`, `handoff`, `learning-distill`,
  `session-title`, `signoff-review`) all omit it. Whether to backfill them is
  a separate call and not obvious in every case — `demo-capture` makes no `gc`
  calls and uses no Gas City vocabulary, while `gc-demo-script` and
  `learning-distill` shell out to nothing and are still conceptually welded to
  beads — so the convention is described here before anything enforces it.
...
> **Open item — the manifest does not exist yet.** There is no
> `.claude-plugin/marketplace.json` in this pack. The section below states the
> contract it will have to satisfy, not a file you can read today. It is
> deliberately not stubbed: no skill currently declares itself portable, so a
> stub would be an empty list and premature. Create it in the same change that
> exports the first portable skill, and turn this note into a flat statement
> then.
...
- Every skill in `marketplace.json` declares portable `compatibility`, and
  no Gas-City-bound or agent-scoped skill appears in it. (Vacuous until the
  manifest exists — see the open item above.)
