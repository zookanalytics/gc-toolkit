...
| Write a gate verdict (`check.<g>`) | `assets/scripts/signoff.sh` | verdict bound to the dispatch-pinned `reviewed_oid`, which must be a full 40-hex oid and must still be on the branch; the review bead still open; that same commit written back to the review bead and read back before the marker is stamped | `--approve` a PR (the city never approves its own); write a verdict for a check it did not run; stamp an abbreviated oid; write a verdict for a commit a rebase took off the branch; record a verdict on a closed review bead; stamp a marker for a commit the review bead does not record — since the city posts no approval, nothing else could resolve it |
