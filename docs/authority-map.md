...
| Close a visit | converse, at the end of a sitting | outcome recorded (`gc.outcome`) | close a subject that still carries unlanded work — a non-closed `merge_result` (`abandoned`, `pull_request`, …) stays open, routed to human, and closing it recreates the I5 closed-but-unlanded shape `lifecycle.sh reopen` and `check-closed-implies-landed` exist to catch. Disposing of a NO-work subject on the sitting's operator-agreed ruling is the `Close-with-successor` row's power, not this one |
| Close-with-successor (disposition) | `assets/scripts/bead-rehome.sh`, called by any actor with the evidence (converse dispositions, operator re-homes, the proactive first reaction's `superseded` exit, a polecat whose work another bead delivered) | the writer re-establishes the evidence itself, for every caller. All kinds: `gc.superseded_by` + store stamped and read back; the origin carries no unlanded work (`merge_result` empty/absent or `merged`), is not a review/step/workflow bead, and is not `in_progress` under another actor; not already pointed at a different successor. The evidence kinds `fixed-upstream`/`duplicate` additionally: the successor resolves in the SAME store and is closed or `work_outcome=shipped`, and the origin did no work — `gc.work_outcome=no-op` (accepted even beside a rebase/rework twin's leftover branch) or no work-product key. `--check` evaluates all of it and writes nothing, so a caller can gate a release on it. converse and the superseded exit stamp `gc.work_outcome=no-op` and name a successor, or `not-needed` with a visit as the evidence | bare-close a re-homed bead; close over unlanded work; judge a successor that lives in another store |
...
| Route work (`gc.routed_to`) | `gc sling` (runtime); direct stamp only where a bare sling would be hijacked (documented at each site). The proactive pool routes a subject RAW (`gc sling --no-formula`), claims it by the routed tier (`gc hook --claim`), and releases it with its own write; `gc.execution_routed_to` is core's pour record, written and cleared only by core, never by the pack. CLEARING `gc.routed_to` is the release half of a handoff, not a routing decision: every terminal arm of `mol-polecat-work` — refinery handoff, `auto_push=false` halt, store-only exit — clears it in the same exit that unassigns the bead | a clear rides a terminal exit that has already recorded where the work went: a branch handed to the refinery, a branch left ready, or a note naming what was filed | route across rig stores; write or clear `gc.execution_routed_to` from the pack; clear the route on a bead this session does not hold, or on one it is still working |
