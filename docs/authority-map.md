...
| Close a step bead | the session executing it, via `assets/scripts/step-close.sh` | `(gc.root_bead_id, gc.step_ref)` — the assignee alone repeats across every molecule an agent ran | close a step in another molecule; close `workflow-finalize` |
