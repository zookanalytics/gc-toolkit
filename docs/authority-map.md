...
| Retire a signoff round cap | `signoff.sh reset <anchor> --reason <why>` on a ruling | a recorded reason; a rework ledger that reads, naming at least one round for the floor to come from; the cap's own pairing still standing — `merge_hold` reading exactly the literal string `signoff_cap` AND a non-empty `signoff_cap=<gate>` beside it; no live demand (`gc.demand_for=<anchor>`) — never the presence of `gc.takeaway`, which outlives the sitting that stamped it; every key it wrote read back | retire a park no `signoff_cap` claims, or one a sitting is still waiting on a person for; read a takeaway as a hold; lift the hold without advancing `signoff_round_floor` (the next pass re-caps); treat an operator's own `merge_hold=true` as the cap's to retire — only the `merge_hold=signoff_cap` pairing is |
...
| Close a no-work subject (validated) | `formulas/mol-validate-close.toml`, run by a capable pool a first reaction's `close` disposition routes to | the reaction's no-work conclusion re-derived against live state (already fixed with nothing to merge, or the bead names nothing to do); `gc.work_outcome=no-op` and a note stamped; the subject unassigned and carrying no non-closed `merge_result` | close a bead carrying a non-closed `merge_result` (the refinery's, on a verified merge) or one whose close needs a successor (`bead-rehome.sh`'s); close on the reaction's reason alone without its own check — any doubt files a visit instead |
...
| Close a dead molecule's chain — every live member under a closed root: its steps (including `workflow-finalize`) and the root | `assets/scripts/dead-molecule-dispose.sh` (`orphan-dispose.sh` routes a closed-root chain to it) | the root resolves and reads `status=closed`; every member de-routed before the first close; each close read back | close under a root that is not `closed` or will not read; close a chain holding a `branch`/`merge_result` bead — only the refinery closes that anchor, on a verified merge |
...
| Close-with-successor (disposition) | `assets/scripts/bead-rehome.sh` callers (mechanik, converse, `duplicate-sweep.sh`, and `pr-facts.sh` consummating a pre-recorded PR-close disposition) | every caller: `gc.superseded_by` + store stamped and read back. `duplicate-sweep.sh`, per bead: the `duplicate_of` successor resolves in this store and is closed or shipped; the duplicate recorded no work, proved by `work_outcome=no-op` or by carrying no work-product key at all; nobody else owns it: unassigned, not `in_progress`, not a review bead, not a step bead or workflow root, not already pointed at a different successor. `converse`, per subject: the sitting concluded, with operator agreement, that it should close (a recommend-close visit is the common trigger). A no-work subject it disposes directly by the no-work test — `merge_result` empty/absent or `merged`, `gc.work_outcome=no-op` or no work-product key, unassigned, not a review/step/workflow bead — stamping `gc.work_outcome=no-op` and a successor, or `not-needed` with this sitting's visit as the evidence. A subject with an in-flight PR it retires instead: the PR is closed and the anchor disposed as superseded in the same act, so the close stands on a disposed anchor, never an open one | bare-close a re-homed bead; `duplicate-sweep.sh`: dispose on the `duplicate_of` stamp alone, or judge a successor that lives in another store; `converse`: change a repo (code or commits route to a molecule); bare-close a subject that still carries a non-closed `merge_result` without retiring its PR |
