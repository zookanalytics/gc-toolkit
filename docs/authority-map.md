...
| Write a gate verdict (`check.<g>`) | `assets/scripts/signoff.sh` | verdict bound to the dispatch-pinned `reviewed_oid`, which must be a full 40-hex oid and must still be on the branch | `--approve` a PR (the city never approves its own); write a verdict for a check it did not run; stamp an abbreviated oid; write a verdict for a commit a rebase took off the branch |
