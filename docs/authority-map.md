...
| Close-with-successor (disposition) | `assets/scripts/bead-rehome.sh` callers (mechanik, converse, a proactive first reaction's `superseded` exit, and `pr-facts.sh` consummating a pre-recorded PR-close disposition) | every caller goes through `bead-rehome.sh`, which gates its own evidence (`--check` evaluates and writes nothing): `gc.superseded_by` + store stamped and read back. A proactive first reaction's `superseded` exit, per bead: the `fixed-upstream`/`duplicate` successor resolves in this store and is closed or shipped; the origin recorded no work, proved by `work_outcome=no-op` or by carrying no work-product key at all; nobody else owns it: unassigned, not `in_progress` under another session, not a review bead, not a step bead or workflow root, not already pointed at a different successor. `converse`, per subject: the sitting concluded, with operator agreement, that it should close (a recommend-close visit is the common trigger). A no-work subject it disposes directly by the no-work test — `merge_result` empty/absent or `merged`, `gc.work_outcome=no-op` or no work-product key, unassigned, not a review/step/workflow bead — stamping `gc.work_outcome=no-op` and a successor, or `not-needed` with this sitting's visit as the evidence. A subject with an in-flight PR it retires instead: the PR is closed and the anchor disposed as superseded in the same act, so the close stands on a disposed anchor, never an open one | bare-close a re-homed bead; a first reaction: take a judgment kind (`re-homed`/`folded`/`not-needed` are the operator's call — take `ruling`), or close past a refused `bead-rehome.sh --check`; `converse`: change a repo (code or commits route to a molecule); bare-close a subject that still carries a non-closed `merge_result` without retiring its PR |
