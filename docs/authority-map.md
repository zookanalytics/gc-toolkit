...
| Write a gate verdict (`check.<g>`) | `assets/scripts/signoff.sh` | the review bead still open; the marker written is the bare lane state (`green` on approve), and `reviewed_oid` is the dispatch pin and posted-artifact audit label — bound to no marker and compared to no head; the dispatch pin still an ancestor of the branch (a rewrite — rebase, amend, force-push, never a fast-forward — refuses both verdicts, clears the pin, and closes the review bead `gc.outcome=superseded` for gate-ensure to re-pour); the closed bead stamped `signoff_verdict=<approve\|request-changes>` | `--approve` a PR (the city never approves its own); write a verdict for a check it did not run; record a verdict on a closed review bead; stamp `green` over a marker still carrying the pre-migration `exception@` shape |
