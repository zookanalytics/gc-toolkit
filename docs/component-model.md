...
Five further checks guard structure that is not an anchor invariant:
...
globally reaches the next process unless the session marks it removed). That is
the whole set: **16 checks, each asserting a live structural property** — none
greps the source for a past fix.
