...
| **I2** | The set of states is closed, and every state has exactly one handler. | **PARTIAL** | The closed half landed 2026-08-24 (tk-jozah0). The 7 `merge_result` literals are declared once, in [work-bead-state-machine.md](work-bead-state-machine.md), each with a *kind* — `handoff` or `disposition` — and `doctor/check-state-space-declared` holds both sides to it: every literal pack code writes is declared, every declared state is written, and every reader that discriminates among two or more names a declared kind-set or declares its own default. The hand-rolled list that made this FALSE is gone — `refinery-reconcile.sh` now tests *absence*, which was its own stated predicate all along; the two lists that remain a pass's own judgement say so in a `merge-result-reader:` comment. What is still open is the **second** clause: `quiesce-completed-workflows.sh` declares that 4 of the 5 dispositions fall through its terminal test, so those states have no handler there. Declared, not yet handled → `check-state-space-declared` + tk-62a2pf |
...
**Five UNCHECKED invariants — I1, I4, I5, I7, I9 — are the output of this
