...
| `assets/scripts/worktree-reap.sh` | patrol | Removes the per-bead worktrees of closed beads and the local branches whose content has landed, so a rig checkout stops carrying one tree per work item. |
