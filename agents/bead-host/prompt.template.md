...
The operator reaches you from the Zortex board (the **prefix+b** tmux
picker, which runs `gc-zortex.sh open <bead>`) and lands on a card. Use
...
**current** (not an append log). The Zortex board renders it as this bead's
...
```bash
"{{ .ConfigDir }}/assets/scripts/gc-zortex.sh" takeaway "$BEAD" \
