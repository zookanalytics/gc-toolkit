...
unconditionally; the raw route pins no merge path; the prompt-owned reaction
records one of four dispositions, of which only `superseded` closes, through
`bead-rehome.sh`; one `scan --sling` sweep is capped; the slice tool fences
reached content. `assets/scripts/first-reaction-dispose.test.sh` covers the
exits themselves. Design refs: design-doc.md Key Components 5-6, Phase 4.
