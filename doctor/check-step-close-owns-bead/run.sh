...
#
# docs/seed-audit/: the ONE exception inside docs/, and it is a scope rule
# rather than a softening of the one above. That tree is generated by
# assets/scripts/render-seed-audit.sh — a rendered mirror of prompts and formula
# recipes, not prose telling anyone to do anything. Two things follow. It reports
# duplicates: whatever this check would find there, it already finds in the
# source the render read. And the findings it adds are unfixable in place —
# `docs/seed-audit/formulas/mol-shutdown-dance.md` carries a `bd close
# "$GC_BEAD_ID"` line from the builtin `core` pack, which this repo does not own,
# and editing the artifact only survives until the next render. Excluding it
# keeps the check pointed at files somebody can actually change (tk-yhwfv.3).
is_excluded() {
    case "$1" in
        */base-snapshots/*) return 0 ;;
        */specs/*) return 0 ;;
        */docs/seed-audit/*) return 0 ;;
        *) return 1 ;;
    esac
}
