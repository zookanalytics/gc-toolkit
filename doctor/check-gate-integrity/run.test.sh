...
store "$(bead g-12 '{"merge_result":"pull_request","check_set":"codex","branch":"polecat/g-12","check.codex":"green"}')"
OUT=$(RIGS_FAIL_TIMES=2 run_check); RC=$?
eq "$RC" "0" "a transient \`gc rig list\` failure is retried, then the scan runs"
has "$OUT" "OK:" "the recovered run reports the gate-integrity verdict, not the enumeration abort"
