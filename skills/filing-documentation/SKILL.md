...
naming, the no-archiving rule, frontmatter — live in
`docs/file-structure.md`, which is the authority. This skill applies
those rules at write-time; it is not a second copy of them.
...
*what was considered while doing this bead* is local. `docs/file-structure.md`
carries the tier rules in full, including the bead-ID-alone directory
naming.
...
timestamps, cross-doc references, the no-archiving rule — read
