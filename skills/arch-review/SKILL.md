...
An architectural objection is usually a decision, not a defect. Rework
children are for defects: a specific fix, on a named line, that the author
can make. A disagreement about where something belongs, or whether it should
exist, cannot be resolved by handing it back — that is the reviewer-fatigue
loop the foundation forbids, and it converges on nothing.

- **`request-changes`** when you can name the file, the line, and the change
  that would satisfy the gate. That is a defect and it loops.
- **`escalate`** when the finding is a choice: two defensible placements, a
  primitive that would have to move, a boundary that would have to be
  redrawn. This stamps `check.arch=exception@<head>` and files one visit that
  frames the choice for a human. The merge stays held until the head moves or
  the operator acts.

```bash
signoff.sh --review-bead "$REVIEW_BEAD" --verdict escalate \
  --reviewed-oid "$REVIEWED_OID" --notes-file findings.md
```

Frame the escalation as a decision with options, not as an objection: what
the change does, which contract it strains, what the alternatives cost, and
what you would do. A visit that only says "this feels wrong" spends the
